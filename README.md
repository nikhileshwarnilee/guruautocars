...
3. Apply upgrade scripts (idempotent):
   ...
   - `mysql -u root guruautocars < database/job_card_advanced_flow_upgrade.sql`
   - `mysql -u root guruautocars < database/odometer_flow_upgrade.sql`
   ...
   - `mysql -u root guruautocars < database/estimate_module_upgrade.sql`
   ...
   - `mysql -u root guruautocars < database/payroll_expense_upgrade.sql`
   - `mysql -u root guruautocars < database/financial_control_upgrade.sql`
...
- Customers
- Vehicles
- Estimates (Draft -> Approved/Rejected -> Converted)
- Job Cards
- Inventory
